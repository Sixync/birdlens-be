...
import (
	...
	"net/url"
	"strconv"
	// Logic: Import the "strings" package to use its TrimSpace function.
	"strings"
	"time"
...
func (app *application) registerHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Logic: Trim whitespace from the email and password fields immediately after decoding.
	// This ensures that any accidental spaces sent by the client are removed before
	// validation and processing, leading to cleaner data and fewer potential bugs.
...
func (app *application) validRegisterUserReq(ctx context.Context, req auth.RegisterUserReq) (bool, string, error) {
	emailExists, err := app.store.Users.EmailExists(ctx, req.Email)
	if err != nil {
		return false, "", err
	}
	if emailExists {
